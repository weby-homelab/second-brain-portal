...
- `Dockerfile`: now `pip install --constraint ... "/tmp/wheels/power_framework-3.7.4-py3-none-any.whl[semantic,rerank]"` + `sha256sum -c` + `python -c "assert find_spec('onnxruntime'); assert find_spec('fastembed')"` proof before `rm -rf /tmp/wheels`.
...
- `compatibility.json` remains candidate until publication (will be promoted to `published` with `release_tag=v0.7.12` + `digest` after public readback — see post-publication metadata (S14)).
...
- **GUI 0.7.12:** tag `v0.7.12` → Merge commit (to be signed), `power-gui==0.7.12`, `Requires-Dist power-framework==3.7.4`, wheel/sdist deterministic (dual build `cmp` identical, `normalize_sdist.py`), `Requires-Python >=3.13,<3.15`
...
- S16 rebuild release truth from clean schemas (no `NO-GO+stable` in one object, no old `0.7.7` fields, no `5abe587` mismatch, no `null` digest after published).
...
`0.7.11` remains **published but not Suite-certified**

**Candidate status:** GUI `0.7.12` candidate, `POWER 3.7.4` exact pair, publication pending, Suite certification pending — do not claim Stable in prepublication. (historical, do not delete/rewrite) per Special Rule 39; `0.7.12` is the forensic corrective target.
